...
def plot_single_BPM_data(bpm_name, sdds_model, sdds_experimental, output_file=None):
    ...
    tx.set_xlabel('$Q_x$ [tune units]')
    ay.set_xlabel('Number of turns')
    ty.set_xlabel('$Q_y$ [tune units]')
    ax.set_ylabel('X [mm]')
    ...
    ax.plot(bpm_model_data['X'][0:900], label='10k particle tracking')
    # tx.plot(fft_tune_model, fft_bpm_model[0], label='FFT Model', color='r')
    ay.plot(bpm_model_data['Y'][0:900], label='10k particle tracking')
...
def do_plot(sdds_experimental, output_file_path=None, sdds_model='/afs/cern.ch/work/f/fcarlier/public/Beta-Beat.src/analysisCode/multiParticleSimulation/test/testagain.sdds', bpms_to_plot=['BPM.33R3.B2']):
