...
def make_plot(fileNames, columnsX, columnsY, labels, colors, division_factors=[]):
    ...
    for i in range(len(fileNames)):
        try:
            ...
            ax.plot(data[0], data[1], label=labels[i], color=colors[i], zorder=0)
            ...
            ax.set_xlabel('S $[m]$')
            ax.set_ylabel('Complex line amplitude')
        except KeyError:
...
if __name__ == '__main__':
    modelPath = '/afs/cern.ch/user/r/rwestenb/workarea/11_12_injectionMD/modelB2inj_new_detuning_matched/analysis/'
    measurementsPath  = '/afs/cern.ch/work/f/fcarlier/public/MD12/correctedSettings/Beam2@Turn@2012_06_25@04_00_22_346_0/'
    ...
    fileNames = [
        os.path.join(measurementsPath, 'getllm/f1102_line.out'),
        os.path.join(measurementsPath, 'getllm/f2020_line.out'),
        os.path.join(modelPath, 'results/f1102_line.out'),
        os.path.join(modelPath, 'results/f2020_line.out')
    ]
    ...
    labels = ['Measurement $f_{1102}$', 'Measurement $f_{2020}$', 'Model $f_{1102}$', 'Model $f_{2020}$']
    colors = ['r', 'g', 'b', 'y']
    ...
    make_plot(fileNames, columnsX, columnsY, labels, colors, division_factors=division_factors)
